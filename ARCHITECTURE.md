...
本文件記錄 Pitch-A-Seat 專案的架構設計決策、技術實作細節、以及開發過程中的問題解決方案。
...
‼️ 推薦系統已於 2026-07 升級為可量測的個人化系統,完整設計、評估與決策見 [RECOMMENDATION_SYSTEM](./RECOMMENDATION_SYSTEM.md)
...
### Lambda 與 SQS 的整合機制 (📝 SQS + Lambda + DLQ)
...
### 舉例：票券上架流程 POST /api/sell_tickets
...
### 安全性設計：信任邊界 + Race Condition 的組合防護
...
#### Q4：decode_responses=True 是做什麼的？
...
| 傳統問題             | Docker 解決方案              |
| -------------------- | ---------------------------- |
| 「我的電腦可以跑啊」 | 環境被容器包起來，到哪都一樣 |
| Python 版本不同      | Dockerfile 指定版本          |
| 套件版本衝突         | 每個容器有獨立的套件         |
| 環境設定不同         | 用 docker-compose 統一設定   |
...
│  │  ┌───────────────────────────────────────────────────────────────────┐  │    │
│  │  │                                                                   │  │    │
│  │  │  - Python 3.11（Dockerfile 指定的）                               │  │    │
│  │  │  - boto3==1.35.0（requirements.txt 指定的）                       │  │    │
│  │  │  - 只有這個專案需要的套件                                         │  │    │
│  │  │                                                                   │  │    │
│  │  │  這裡面的東西和 Mac 本機完全無關！                                │  │    │
│  │  │                                                                   │  │    │
...
- 程式碼在你的電腦上編輯（用 VS Code）
- 程式在容器裡執行（用容器裡的 Python 和套件）
- 套件安裝在容器裡（不會影響你電腦上的其他專案）
...
| 情境           | 方式 | 說明                             |
| -------------- | ---- | -------------------------------- |
| **本機開發**   | 掛載 | 你改檔案，容器即時看到變化       |
| **CI/CD 部署** | 複製 | 把程式碼複製進映像，變成固定版本 |
...
│  ┌───────────────────┐              ┌───────────────────┐                       │
│  │                   │              │                   │                       │
│  │  app.py ←─────────┼──── 掛載 ────┼──→ /app/app.py    │                        │
│  │  （你改這裡）       │   （同一個檔案）│   （容器看這裡）    │                       │
│  │                   │              │                   │                       │
│  └───────────────────┘              └───────────────────┘                       │
│                                                                                 │
│  你在 Mac 上改 app.py → 容器裡的 app.py 也同時變                                    │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
...
│       │                                                                         │
│       └── 把程式碼「複製」進 Docker 映像                                            │
│           （不是掛載，是複製一份進去，變成映像的一部分）                                │
│                                                                                 │
│  這個映像推到 Docker Hub，EC2 拉下來執行                                            │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| 檔案                      | 用途     | 關鍵設定                             |
| ------------------------- | -------- | ------------------------------------ |
| `docker-compose.yml`      | 本機開發 | `build: .`（用本地程式碼 build）     |
| `docker-compose.prod.yml` | EC2 部署 | `image: xxx`（用 Docker Hub 的映像） |

```yaml
# docker-compose.yml（本機開發）
services:
  web:
    build: # ← 用本地 Dockerfile build
      context: .
      platforms:
        - linux/amd64
    ports:
...
```yaml
# docker-compose.prod.yml（EC2 部署）
services:
  web:
    image: elsachung/project-practice:latest # ← 直接用 Docker Hub 映像
    ports:
      - "8080:8000"
    env_file:
      - .env
...
│       └── 測試失敗 ✗ → 停止，不部署                                             │
│                                                                                 │
│  【GitHub Actions - Deploy 階段】                                                │
│       │                                                                         │
│       ├── docker build（用 Dockerfile 建立映像）                                │
│       ├── docker push（推到 Docker Hub）                                        │
│       │                                                                         │
│       └── SSH 到 EC2 執行：                                                     │
│           ├── docker system prune -af（清理磁碟空間）                           │
│           ├── docker-compose down（停止並刪除舊容器）                           │
│           ├── docker-compose pull（拉最新映像）                                 │
...
**原因**：每次 CI/CD 都會拉新映像，舊的沒刪除導致累積。

**解決**：在部署前先清理

```yaml
script: |
  docker system prune -af           # 清理舊映像、舊容器
  docker-compose -f docker-compose.prod.yml down
  docker-compose -f docker-compose.prod.yml pull
  docker-compose -f docker-compose.prod.yml up -d
```

#### 問題 2：docker-compose 版本相容性

```
KeyError: 'ContainerConfig'
```

**原因**：舊版 docker-compose (1.29.2) 讀取新版 Docker 容器資訊時 API 不相容。

**解決**：先 down 再 up，避免讀取舊容器資訊

```bash
docker-compose down    # 先刪除舊容器
docker-compose up -d   # 再建立新容器
```
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  每次 CI/CD 執行 docker build：                                                 │
│                                                                                 │
│  FROM python:3.11-slim          ← 從乾淨的 Python 映像開始                      │
│       │                                                                         │
│       ▼                                                                         │
│  pip install -r requirements.txt ← 安裝指定版本的套件                           │
│       │                                                                         │
│       ▼                                                                         │
│  全新的環境，沒有「之前裝的舊套件」問題                                          │
│                                                                                 │
...
- name: Deploy to EC2
  ...
  with:
    ...
    script: |
      cd ~/app
      docker system prune -af                           # 清理磁碟
      docker-compose -f docker-compose.prod.yml down    # 刪除舊容器
      docker-compose -f docker-compose.prod.yml pull    # 拉新映像
      docker-compose -f docker-compose.prod.yml up -d   # 啟動新容器
```

| 指令                      | 解決什麼問題                  |
| ------------------------- | ----------------------------- |
| `docker system prune -af` | 避免磁碟空間不足              |
| `docker-compose down`     | 避免版本相容性問題、Port 衝突 |
| `docker-compose pull`     | 確保拉到最新映像              |
| `docker-compose up -d`    | 啟動新容器                    |

---

### docker-compose 版本相容性問題
...
│       ├── 【沒有舊容器】                                                        │
│       │       │                                                                 │
│       │       └── 直接創建新容器 ✓（舊版 docker-compose 可以做到）              │
│       │                                                                         │
│       └── 【有舊容器】                                                          │
│               │                                                                 │
│               └── 讀取舊容器資訊（ContainerConfig）                             │
│                       │                                                         │
│                       └── 舊版 docker-compose (1.29.2) 讀不懂                   │
│                           新版 Docker (27.x) 的 API 格式 ✗                      │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

| 操作                 | 舊版 docker-compose 能做嗎？ |
| -------------------- | ---------------------------- |
| 創建新容器           | ✓ 可以                       |
| 讀取舊容器資訊並重建 | ✗ 會報錯（API 不相容）       |
| 刪除舊容器（down）   | ✓ 可以                       |
| 拉映像（pull）       | ✓ 可以                       |
...
script: |
  ...
  docker-compose -f docker-compose.prod.yml down    # 先刪除舊容器
  docker-compose -f docker-compose.prod.yml pull
  docker-compose -f docker-compose.prod.yml up -d   # 創建新容器（不是重建）
```

| 步驟 | 指令                   | 作用                                 |
| ---- | ---------------------- | ------------------------------------ |
| 1    | `docker-compose down`  | 刪除舊容器                           |
| 2    | `docker-compose up -d` | 創建新容器（沒有舊容器，不需要讀取） |

**關鍵洞察**：我們不需要「重建舊容器」，我們只需要「創建新容器」。先把舊的刪掉，docker-compose 就不用煩惱相容性問題。

**經驗**：部署腳本要考慮版本相容性，不能假設環境是理想的。先清理再建立是更穩健的做法。
...
services:
  web:
    ...
    env_file:
      - .env # 容器啟動時，讀取 .env 並注入到容器內
```

#### .env 會被打包進 Docker 映像嗎？
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  【docker build 階段】（打包映像）                                               │
│                                                                                 │
│  Dockerfile 執行：                                                               │
│  - FROM python:3.11-slim                                                        │
│  - COPY requirements.txt .                                                      │
│  - pip install ...                                                              │
│  - COPY . .                ← 複製程式碼，但 .env 被 .dockerignore 排除          │
│                                                                                 │
│  映像裡面「沒有」.env 檔案                                                       │
│                                                                                 │
│  ─────────────────────────────────────────────────────────────────────────────  │
│                                                                                 │
│  【docker-compose up 階段】（執行容器）                                          │
│                                                                                 │
│  env_file:                                                                      │
│    - .env                  ← 這時候才讀取                                       │
│                                                                                 │
│  讀取「執行這個指令的那台機器」上的 .env：                                       │
│  - 本機開發：讀你 Mac 上的 .env                                                 │
│  - EC2 部署：讀 EC2 上的 ~/app/.env                                             │
│                                                                                 │
│  環境變數注入到「執行中的容器」，不會變成映像的一部分                            │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| ------------ | ------------ | ----------------------- |
| 你的 Mac     | ✓ 有         | 你自己建立的            |
| GitHub       | ✗ 沒有       | .gitignore 排除         |
| Docker 映像  | ✗ 沒有       | .dockerignore 排除      |
| Docker Hub   | ✗ 沒有       | 映像裡沒有              |
| EC2          | ✓ 有         | 你手動建立的 ~/app/.env |
| 執行中的容器 | ✓ 有環境變數 | 從 EC2 的 .env 注入     |
...
```
Docker 映像（可以公開）           .env 檔案（只存在部署環境）
┌─────────────────────────┐      ┌─────────────────────────┐
│  - Python 3.11          │      │  DB_PASSWORD=xxx        │
│  - 你的程式碼           │  +   │  JWT_SECRET_KEY=xxx     │
│  - 套件                 │      │  AWS_SECRET_ACCESS_KEY= │
│                         │      │                         │
│  不包含任何機密！       │      │  只存在執行環境         │
└─────────────────────────┘      └─────────────────────────┘
                    │                        │
...
- 映像可以放 Docker Hub，不怕洩密
- 不同環境（開發、測試、生產）用不同的 .env
- 換密碼只要改 .env，不用重新 build 映像

---

### CI/CD 中 Build 和 Run 階段的差異

#### 發生在不同的地方

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                    CI/CD 中的 Build 和 Run 階段                                  │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  【GitHub Actions Runner】（GitHub 的雲端機器）                                  │
│                                                                                 │
│  docker build                                                                   │
│       │                                                                         │
│       ├── 讀取 Dockerfile                                                       │
│       ├── 執行 COPY . .（參考 .dockerignore，跳過 .env）                        │
│       ├── 這台機器上根本沒有你的 .env（GitHub 上沒有）                          │
│       └── 產生映像，推到 Docker Hub                                             │
│                                                                                 │
│  ─────────────────────────────────────────────────────────────────────────────  │
│                                                                                 │
...
│  docker-compose up -d                                                           │
│       │                                                                         │
│       ├── 讀取 docker-compose.prod.yml                                          │
│       ├── 看到 env_file: .env                                                   │
│       ├── 讀取 EC2 上的 ~/app/.env（這個檔案存在）                              │
│       └── 把環境變數注入到容器                                                  │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| 階段                | 執行位置              | 有 .env 嗎？ | 原因                                     |
| ------------------- | --------------------- | ------------ | ---------------------------------------- |
| `docker build`      | GitHub Actions Runner | ✗ 沒有       | GitHub 上沒有 .env（被 .gitignore 排除） |
| `docker-compose up` | EC2                   | ✓ 有         | 你手動建立的 ~/app/.env                  |

**不是「選擇讀不讀」，而是「那個位置有沒有 .env」。**

---

### Deploy.yml Actions 語法解析
...
| Action                          | 作者                   | 功能                                   |
| ------------------------------- | ---------------------- | -------------------------------------- |
| `actions/checkout@v4`           | GitHub 官方            | 把程式碼下載到 Runner                  |
| `actions/setup-python@v5`       | GitHub 官方            | 安裝指定版本的 Python                  |
| `docker/login-action@v3`        | Docker 官方            | 登入 Docker Hub                        |
| `docker/setup-buildx-action@v3` | Docker 官方            | 設定 Docker Buildx（支援多平台 build） |
| `docker/build-push-action@v5`   | Docker 官方            | Build 映像並 Push 到 Docker Hub        |
| `appleboy/ssh-action@v1.0.3`    | appleboy（台灣開發者） | SSH 連線到遠端主機執行指令             |

#### runs-on 選項

| 選項             | 說明                          |
| ---------------- | ----------------------------- |
| `ubuntu-latest`  | 最新版 Ubuntu（免費額度最多） |
| `ubuntu-22.04`   | 指定 Ubuntu 22.04             |
...
#### Q2：Redis 和 S3、SQS 的 Singleton 有什麼不同？
...
### HTTPException 的處理位置 (Model 層設計原則)
